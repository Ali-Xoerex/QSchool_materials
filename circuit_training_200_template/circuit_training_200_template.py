...
def find_max_independent_set(graph, params):
    ...
    probabilities = max_set(params)
    max_indie_set = np.argmax(probabilities)
    bit_string = '{0:0'+str(NODES)+'b}'
    max_ind_set = list(bit_string.format(max_indie_set))
    max_ind_set = [i for i in range(NODES) if max_ind_set[i] == "1"]    
    # QHACK #
